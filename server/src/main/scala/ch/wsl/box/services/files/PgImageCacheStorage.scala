...
class PgImageCacheStorage extends ImageCacheStorage {
  override def save(fileId: FileCacheKey, data: Array[Byte])(implicit ex:ExecutionContext): Future[Boolean] = Auth.adminDB.run{
    BoxImageCache.Table.insertOrUpdate(BoxImageCache.BoxImageCache_row(fileId.asString,data))
  }.map(_ => true)

  override def delete(fileId: FileCacheKey)(implicit  ex:ExecutionContext): Future[Boolean] = Auth.adminDB.run{
    BoxImageCache.Table.filter(_.key === fileId.asString).delete
  }.map(_ => true)

  override def get(fileId: FileCacheKey)(implicit ex:ExecutionContext): Future[Option[Array[Byte]]] = Auth.adminDB.run{
    BoxImageCache.Table.filter(_.key === fileId.asString).take(1).result
  }.map(_.headOption.map(_.data))

  override def clearField(id: FileId)(implicit ex: ExecutionContext): Future[Boolean] = Auth.adminDB.run{
