...
object Form {
  ...
  case class Form_row(id: Option[Int] = None, name: String, table:String, description: Option[String] = None, layout: Option[String] = None)
  ...
  class Form(_tableTag: Tag) extends profile.api.Table[Form_row](_tableTag, "form") {
    def * = (Rep.Some(id), name, table, description, layout) <> (Form_row.tupled, Form_row.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(id), name, table, description, layout).shaped.<>({r=>import r._; _1.map(_=> Form_row.tupled((_1, _2, _3, _4, _5)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
