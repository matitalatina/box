...
object Log {
  ...
  class Logs(_tableTag: Tag) extends profile.api.Table[Log_row](_tableTag, "log") {
    ...
    def ? = (Rep.Some(id), filename, classname, line, message,timestamp).shaped.<>({r=>import r._; _1.map(_=> Log_row.tupled((_1, _2, _3, _4, _5)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
