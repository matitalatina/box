...
//  def schema(kind:String, lang:String, entity:String): Future[JSONSchema] = client.get[JSONSchema](s"/${EntityKind(kind).entityOrForm}/$lang/$entity/schema")
  ...
  def update(kind:String, lang:String, entity:String, id:JSONID, data:Json):Future[JSONID] = client.put[Json,JSONID](s"/${EntityKind(kind).entityOrForm}/$lang/$entity/id/${id.asString}",data)
